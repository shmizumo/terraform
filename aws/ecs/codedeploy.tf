...
resource "aws_codedeploy_deployment_group" "ecs_bluegreen" {
  ...
  #deployment_config_name = "CodeDeployDefault.ECSCanary10percent5Minutes" #最初の増分でトラフィックの 10% を移行します。残りの 90% は 5 分後にデプロイします。
  ...
  ecs_service {
    ...
    service_name = aws_ecs_service.test_service.name
  }
